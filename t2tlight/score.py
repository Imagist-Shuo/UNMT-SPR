...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Score using existing NMT models",
        usage="score.py [<args>] [-h | --help]"
    )
...
def main(args):
    ...
    params = import_params(args.model, model_cls.get_name(), params)
    override_parameters(params, args)

    # Build Graph
    with tf.Graph().as_default():
        model = model_cls(params, model_cls.get_name())
        inputs = read_files(args.input)
        word_counts = count_files(inputs)
        ...
        # Load checkpoint
        tf.logging.info("Loading %s" % args.model)
        var_list = tf.train.list_variables(args.model)
        values = {}
        reader = tf.train.load_checkpoint(args.model)
        ...
        with tf.train.MonitoredSession(session_creator=sess_creator) as sess:
            # Restore variables
            sess.run(assign_op)
            fd = tf.gfile.Open(args.output, "wb")

            index=0
            while not sess.should_stop():
                results = sess.run(scores)
                index +=1
                print("Finish Batch %d" % index)
        ...
        print("Source->Target Translation PPL: %.2f" % math.exp(-LLH/word_counts[1]))
